...
Breakpoints come from a single source, [_breakpoints.scss](src/assets/styles/setup/_breakpoints.scss), and are used through the `until` / `from` / `between` mixins rather than raw media queries: mobile ≤ 480px, tablet 600–1024px, desktop 1200–1920px, ultra ≥ 2560px, plus a 4K step at 3840px. Legacy `for-mobile-layout` style mixins still exist but are aliases onto the same values — one system, two vocabularies. Every breakpoint in the header comes from that scale; the category nav gives way to the hamburger drawer at `$bp-md` (1024px).

**The masthead carries the store; the bar carries the catalogue.** The blue band was a slab — 128px of blue holding one 96px logo, under a 48px grey strip carrying two links, for 176px of chrome above the bar that does the work. It is a `1fr auto 1fr` grid now: utility links on the inline-start edge, the mark centred, the branch locator and the social accounts opposite. Symmetric side columns are safe here precisely because the middle is a fixed-width logo. Below 600px the band is the mark alone — everything its edges carry is already in the drawer or the footer. Header chrome at desktop went 242px → 178px.

The sticky bar is its own three-column grid, `auto minmax(0, 1fr) auto`, with the columns **pinned by `grid-column` rather than auto-placed** — below the nav breakpoint `.nav-wrap` is `display: none`, and auto-placement walked the action cluster into the `1fr` middle track where it stretched to 285px of a 375px bar.

**The departments are the middle column, and the slack splits around them.** As two flex clusters against opposite gutters, every pixel the nav did not need collected into one hole beside it: 220px at 1600, 514px at 1920, and 437–507px between 1024 and 1200 where the nav was not rendered at all. Centring the nav in its own column splits the same slack evenly either side. The earlier arrangement here — a fixed-width *search field* in the middle with `1fr` sides — failed for the opposite reason: `1fr` sides starved the nav until it collided with the field. Sides are `auto` and the middle is `1fr`, so neither can starve.

**The nav's scale grows with the viewport, it does not step.** Type, link padding and gap are all `clamp()`ed against `vw`. The old fixed 1rem gap and 12px padding, against a font that hit its 16px ceiling at 1000px wide, froze the nav at 801px — so everything past 1440 became slack rather than legibility. It now condenses to 623px at 1024 (44px of headroom in its track) and returns to exactly 801px at 1600.

The mini-logo **reserves its 50px slot at rest and only fades in**. Animating `width` from 0 changed the start column's size on scroll, which with a centred nav would drag all eight departments sideways as the mark arrived — and it needed a negative margin to cancel the flex gap the collapsed item still consumed. Measured shift on scroll is now 0px at both 1024 and 1600.
...
### Navigation
- **Desktop:** An opaque sticky bar under the blue masthead, laid out as three grid columns — the mark's reserved slot, the centred departments, the shopper's own controls. Category links are blue, weight 600, in 52px-tall rows; hovering fills the row with High-Voltage Yellow and rotates the chevron 180°. Dropdowns are white panels at `8px` radius carrying the overlay shadow, laid out as an 8-row column-flow grid.
...
- **Mobile (<1024px):** Nav collapses to a 44px hamburger at `rgba($clr1, 0.08)`. The drawer slides from the right (RTL) at `min(84vw, 360px)` over a `rgba(0,0,0,0.45)` scrim, with accordion subcategories animating via `max-height`.
- **Stacking:** `.nav-wrap` used to carry `transform: translateY(0)` for its collapse animation, and any non-`none` transform creates a stacking context — which made the dropdown's own `z-index: 9999` inert. The transform went with the collapse, but the nav layer keeps its `position: relative; z-index: 20`: it is what holds open menus above the rest of the bar.
  **Known defect:** `.search-overlay` is `z-index: 30`, above the nav's 20 — so with the search panel open, a department dropdown opens *behind* it and only its lower half is visible. This predates the grid rework and is unresolved; the number in this document said 10, which is what made the pairing look correct on paper. Either the overlay drops below the nav layer, or opening search should close the dropdowns.
